...
class Settings_Revisions_Plugin {
	...
	function __construct() {
		register_activation_hook( SETTINGS_REVISIONS_PLUGIN_FILE, array( $this, 'activate' ) );
		register_deactivation_hook( SETTINGS_REVISIONS_PLUGIN_FILE, array( $this, 'deactivate' ) );
		...
		$this->post_type = new Settings_Revisions_Post_Type( $args );
		$this->customizer_integration = new Settings_Revisions_Customizer_Integration( $args );
		if ( defined( 'WP_CLI' ) && WP_CLI ) {
			$this->cli = new Settings_Revisions_Command_Line_Interface( $args );
		}
		// @todo This is not currently applicable because we don't support pending or future revisions
		//$this->settings_filtering = new Settings_Revisions_Filtering( array(
		//	'plugin' => $this,
		//));
	...
	function get_plugin_path_url( $path = null ) {
		$plugin_dirname = basename( dirname( SETTINGS_REVISIONS_PLUGIN_FILE ) );
	...
	function load_textdomain() {
		...
		load_plugin_textdomain( 'settings-revisions', false, dirname( plugin_basename( SETTINGS_REVISIONS_PLUGIN_FILE ) ) . trailingslashit( $this->get_meta_data( 'DomainPath' ) ) );
	}
	...
	function get_meta_data( $key = null ) {
		...
		$data = get_plugin_data( SETTINGS_REVISIONS_PLUGIN_FILE );
		if ( ! is_null( $key ) ) {
			return $data[$key];
		} else {
			return $data;
